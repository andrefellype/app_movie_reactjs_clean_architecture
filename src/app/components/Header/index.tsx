...
const Header: React.FC<{ onMobileNavOpen: () => void, titleHeader?: string | null, menusList: HeaderMenu[] }> = function ({ onMobileNavOpen, titleHeader, menusList }) {
    ...
    return (
        <AppBar position="fixed" sx={{ width: { sm: `calc(100% - ${drawerWidth}px)` }, ml: { sm: `${drawerWidth}px` } }}>
            <Toolbar>
                <IconButton color="inherit" aria-label="open drawer" edge="start" onClick={onMobileNavOpen} sx={{ mr: 1, display: { sm: 'none' } }}>
                    <MenuIcon />
                </IconButton>
                ...
                {menusList.map((value, key) => (value.isDesktop !== value.isMobile) ? (
                    <Hidden key={key} lgUp={value.isMobile} mdDown={value.isDesktop}>
                        <HeaderItem icon={value.icon} title={value.title} badgeText={value.badgeText} isNotification={value.isNotification} notifications={value.notifications}
