...
type typeMsgAlert = {
    title?: string | null, icon?: 'success' | 'error' | 'warning' | 'info' | 'question' | null, type: string,
    msgs: string[], confirmButton?: string | null
}
type typeLoading = { statusPattern: false, titlePattern: "" }

const LayoutMain: React.FC<{
    titleHeaderValue: string, msgAlert?: typeMsgAlert | null, cleanMsg?: (() => void) | null,
    isLoading?: typeLoading | null, signOutCallback?: (() => void) | null, menusNavBar: NavBarMenu[],
    menusHeader: HeaderMenu[], urlSignIn: string, isScrollTop: boolean
}> = function ({
    titleHeaderValue, msgAlert, cleanMsg, isLoading, signOutCallback, menusNavBar, menusHeader, urlSignIn,
    isScrollTop
}) {
        ...
        return (
            <Box sx={{ display: 'flex' }}>
                <Header titleHeader={titleHeaderValue} onMobileNavOpen={handleDrawerToggle}
                    menusList={menusHeader} />
                ...
                <Box component="main" sx={{ flexGrow: 1, width: { sm: `calc(100% - ${drawerWidth}px)` } }}>
                    <Toolbar />
                    {(typeof isLoading !== "undefined" && isLoading !== null && isLoading.statusPattern)
                        && <LoadingBackdrop loadingObject={isLoading} />}
                    {isScrollTop && <Fab onClick={scrollToTop} sx={{
                        position: 'fixed', display: visible ? 'visible' : 'none', bottom: 16, right: 16
                    }} color="primary">
