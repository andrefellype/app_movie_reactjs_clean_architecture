...
type NavBarItemModel = {
    ...
    notifications?: {
        text: string | string[], icon: string, cursorClick?: "pointer" | null,
        actionClick?: (() => void) | null
    }[], dropdowns?: {
...
const NavBarItem: React.FC<NavBarItemModel> = function ({
    icon, title, badgeText, notifications, dropdowns, collapsables, redirectUrl, clickOut }) {
    ...
    function getLink() {
        ...
        if (getNotifications().length > 0)
            return <NavBarItemNotifications badgeText={badgeText} icon={icon} notifications={getNotifications()}
                title={title} />
        ...
        if (getCollapsables().length > 0)
            return <NavBarItemCollapsables badgeText={badgeText} icon={icon} title={title}
                collapsables={getCollapsables()} />
        return null
    }
