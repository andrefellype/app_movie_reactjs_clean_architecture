...
type NavBarItemModel = { icon?: string, title?: string, badgeText?: string, dropdowns: { text: string | string[], icon: string, cursorClick?: "pointer" | null, actionClick?: (() => void) | null }[] }
const NavBarItemDropdowns: React.FC<NavBarItemModel> = function ({ icon, title, badgeText, dropdowns }) {
    ...
    return (
        <React.Fragment>
            ...
            <DialogList showDialog={showDropdown} onCloseDialog={() => setShowDropdown(true)} clickCloseDialog={() => setShowDropdown(false)} listDialog={dropdowns} />
        </React.Fragment>
    )
}
