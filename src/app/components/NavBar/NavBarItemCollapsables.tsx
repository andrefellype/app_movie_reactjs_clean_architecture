...
type NavBarItemModel = {
    icon?: string, title?: string, badgeText?: string, collapsables: {
...
const NavBarItemCollapsables: React.FC<NavBarItemModel> = function ({ icon, title, badgeText, collapsables }) {
    ...
    function getLink(collapsable, collapsableKey) {
        if (typeof collapsable.redirectUrl !== "undefined" && collapsable.redirectUrl !== null)
            return <NavBarItemLink key={collapsableKey} icon={collapsable.icon} redirectUrl={collapsable.redirectUrl}
                title={collapsable.text} />
        if (typeof collapsable.actionClick !== "undefined" && collapsable.actionClick !== null)
            return <NavBarItemOnClick key={collapsableKey} icon={collapsable.icon} title={collapsable.text}
                clickOut={collapsable.actionClick} />
        return null
    }
    ...
    return (
        <React.Fragment>
            <ListItem button onClick={() => setShowCollapsable(true)}>
                ...
                {showCollapsable ? <IconList icon={ICON_OBJECT_LIST.EXPAND_LESS_ICON} style={{ marginLeft: 5 }} /> :
                    <IconList icon={ICON_OBJECT_LIST.EXPAND_MORE_ICON} style={{ marginLeft: 5 }} />}
            </ListItem>
