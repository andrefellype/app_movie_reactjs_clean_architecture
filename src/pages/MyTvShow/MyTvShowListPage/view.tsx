...
const MyTvShowListPageView: React.FC<{
    orderDefault: string, showLoading: boolean, myTvShows: any, positionPage: number, countMyTvShow: any,
    changePagination: any, getCategories: any, getCountries: any, actionOrderList: any, actionChangeSearchText: any,
    actionClickFilter: any, actionEraseFilter: any, actionRefreshList: any, actionDeleteRegister: any
}> =
    function ({ orderDefault, showLoading, myTvShows, countMyTvShow, positionPage, changePagination, getCategories,
        getCountries, actionOrderList, actionChangeSearchText, actionClickFilter, actionEraseFilter, actionRefreshList, actionDeleteRegister }) {
        ...
        React.useEffect(() => {
            ...
            if (monthReleaseFilter > 0) {
                const dayMonth = ConvertDate(`${monthReleaseFilter >= 10 ? monthReleaseFilter
                    : `0${monthReleaseFilter}`}_${yearReleaseFilter}`, "totalDayInMonthAndYear")
                const day: { label: string, value: number }[] = []
                for (let d = 1; d <= dayMonth; d++) {
                    day.push({ label: d.toString(), value: d })
                }
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item md={4} xs={12}>
                                    <FormControlFieldSelect labelValue="MÊS" isDisabled={yearReleaseFilter === 0} valueDefault={monthReleaseFilter > 0
                                        ? monthReleaseFilter : ""} selectList={months}
                                ...
                                <Grid item md={4} xs={12}>
                                    <FormControlFieldSelect labelValue="ANO" valueDefault={yearReleaseFilter > 0 ? yearReleaseFilter : ""}
                                        selectList={years} onChangeForm={(e) => setYearReleaseFilter(e.target.value.length > 0 ? parseInt(e.target.value, 10) : 0)} />
                                </Grid>
                            </Grid>
                        </Grid>
                    </Grid>
            ...
            return (<>
                <CardActions className={classes.flex_center}>
                    ...
                    <ButtonSuccess style={{ marginLeft: 10 }} titleIcon={ICON_OBJECT_LIST.ADD_ICON} iconTitleSize="medium"
                        actionClick={() => navigate(URL_MY_TV_SHOW_NEW)} />
                </CardActions>
...
export default MyTvShowListPageView