...
import {
    Card, CardActions, Grid, TableContainer, Paper, Table, TableRow, InputAdornment, IconButton, Dialog,
    DialogContent, DialogActions, Radio, RadioGroup, FormControlLabel, FormControl, Checkbox
} from "@mui/material"
...
const UserListTrashPageView: React.FC<{
    users: any, orderDefault: string, actionChangeSearchText: any, actionClickFilter: any, actionEraseFilter: any,
    actionOrderList: any, actionRefreshList: any, actionDeleteBatch: any, showLoading: any | null, getListScroll: (valueScroll: number) => any[]
}> = function ({ users, orderDefault, actionChangeSearchText, actionClickFilter, actionEraseFilter, actionOrderList,
    actionRefreshList, actionDeleteBatch, showLoading, getListScroll }) {
        ...
        function searchFinish(search: string) {
            actionChangeSearchText(orderField, search, levelFilter, () => setPositionScroll(1))
        }
        ...
        function eraseFilter() {
            ...
            setLevelFilter("")
            setLevelNameFilter("")
            actionEraseFilter(orderField, searchText, () => setPositionScroll(1))
        }
        ...
        function clickFilter() {
            ...
            setIsFilter(false)
            actionClickFilter(orderField, searchText, levelFilterTemp, () => setPositionScroll(1))
        }

        function orderList(order) {
            setOrderField(order)
            setIsOrder(false)
            actionOrderList(order, searchText, levelFilter, () => setPositionScroll(1))
        }

        async function refreshList() {
            actionRefreshList(orderField, searchText, levelFilter, () => setPositionScroll(1))
        }
        ...
        async function deleteBatch() {
            ...
            actionDeleteBatch(stateArrayDeleteBatch.arrayDeleteBatch, orderField, searchText, levelFilter,
        ...
        function filterListUsers() {
            return (<>
                <CardActions className={classes.flex_center}>
                    ...
                    <ButtonDanger style={{ marginLeft: 10 }} titleIcon={ICON_OBJECT_LIST.DELETE_ICON}
                        iconTitleSize="medium" actionClick={() => setIsDeleteBatchShow(true)} />
                        ...
                        <Table aria-label="customized table">
                            ...
                            <TableBodyStyle isLoading={showLoading} colSpanValue={7} listData={users}>
                                {listScroll.map((row, key) => (
                                    <TableRowStyle hover key={key} onClick={() =>
                                        changeDeleteBatch(!(stateArrayDeleteBatch.arrayDeleteBatch.filter(value => value === row._id).length > 0), row._id)}>
...
export default UserListTrashPageView