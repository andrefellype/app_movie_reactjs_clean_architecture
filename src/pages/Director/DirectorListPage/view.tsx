...
import {
    MSG_APPROVED_REVIEWED_REGISTER_QUESTION, MSG_DELETE_REGISTERS_QUESTION,
    MSG_DELETE_REGISTER_QUESTION, URL_DIRECTOR_EDIT, URL_DIRECTOR_NEW
...
const DirectorListPageView: React.FC<{
    directors: any, showLoading: boolean, actionChangeSearchText: any, actionRefreshList: any, actionDeleteRegister: any,
    actionDeleteBatch: any, actionApprovedRegister: any, getListScroll: (valueScroll: number) => any[]
}> = function ({
    directors, showLoading, actionChangeSearchText, actionRefreshList, actionDeleteRegister,
    actionDeleteBatch, actionApprovedRegister, getListScroll }) {
        ...
        async function deleteBatch() {
            ...
            actionDeleteBatch(stateArrayDeleteBatch.arrayDeleteBatch, searchText, () =>
                dispatchArrayDeleteBatch({ type: 'erase' }))
        }
        ...
        function filterListDirectors() {
            return (<CardActions className={classes.flex_center}>
                ...
                <ButtonSuccess style={{ marginLeft: 10 }} titleIcon={ICON_OBJECT_LIST.ADD_ICON} iconTitleSize="medium"
                    actionClick={() => navigate(URL_DIRECTOR_NEW)} />
                <ButtonDanger isDisabled={(!directors || directors.length === 0)} titleIcon={ICON_OBJECT_LIST.DELETE_ICON}
                                        ...
                                        <TableCellStyle align="center">
                                            <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                {!row.reviewed && <ButtonSuccess sizeBtn='small'
                                                    actionClick={() => setIdReviewed(row._id)}
                                                    titleIcon={ICON_OBJECT_LIST.CHECK_ICON} />}
                                                <ButtonIndigo sizeBtn='small' actionClick={() => navigate(`${URL_DIRECTOR_EDIT}/${row._id}`)}
        ...
        function dialogDeleteBatch() {
            return (<Dialog fullWidth maxWidth="sm" open={isDeleteBatch}
                onClose={() => setIsDeleteBatch(true)} aria-labelledby="form-dialog-title">
                <DialogContent>
                        ...
                        <Grid item xs={12}>
                            <ScrollInfiniteList isShowScroll={(!directors || (positionScrollBatch * valueInitialScrollBatch) < directors.length)}
                                ...
                                <TableContainer component={Paper}>
                                    <Table aria-label="customized table">
                                        <TableBody>
                                            {directors && getListScroll((positionScrollBatch * valueInitialScrollBatch))
                                                .map((row, key) => (
                                                    <TableRowStyle hover key={key}
...
export default DirectorListPageView