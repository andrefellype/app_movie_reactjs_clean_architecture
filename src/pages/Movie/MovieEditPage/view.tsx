...
import {
    ButtonGroup, Card, CardActions, Dialog, DialogActions, DialogContent, Grid, IconButton, InputAdornment,
...
const MovieEditPageView: React.FC<{
    streams: any, stream: any, actionChangeSearchStream: any, actionRefreshListStream: any, actionInsertcreateInStream: any,
    actionUpdatecreateInStream: any, actionDeletecreateInStream: any, openStream: any, countries: any,
    country: any, actionChangeSearchCountry: any, actionRefreshListCountry: any, actionInsertcreateInCountry: any,
    actionUpdatecreateInCountry: any, actionDeletecreateInCountry: any, openCountry: any,
    categories: any, actionRefreshListCategory: any, actionChangeSearchCategory: any,
    actors: any, actor: any, actionChangeSearchCast: any, actionInsertcreateInActor: any, actionUpdatecreateInActor: any,
    actionRefreshListCast: any, openActor: any, actionDeletecreateInActor: any, directors: any, director: any,
    actionChangeSearchDirector: any, actionInsertRegisterDirector: any, actionUpdateRegisterDirector: any, actionRefreshListDirector: any,
    openDirector: any, actionDeleteRegisterDirector: any, getMovie: any, isLoading: boolean, update: any
}> = function ({
    streams, stream, actionChangeSearchStream, actionRefreshListStream, actionInsertcreateInStream, actionUpdatecreateInStream,
    actionDeletecreateInStream, openStream, countries, country, actionChangeSearchCountry, actionRefreshListCountry,
    actionInsertcreateInCountry, actionUpdatecreateInCountry, actionDeletecreateInCountry, openCountry,
    categories, actionRefreshListCategory, actionChangeSearchCategory, actors, actor, actionChangeSearchCast,
    actionInsertcreateInActor, actionUpdatecreateInActor, actionRefreshListCast, openActor, actionDeletecreateInActor,
    directors, director, actionChangeSearchDirector, actionRefreshListDirector, actionInsertRegisterDirector,
    actionUpdateRegisterDirector, openDirector, actionDeleteRegisterDirector, getMovie, isLoading, update
}) {
        ...
        function confirmRelease() {
            ...
            if (yearRelease > 0 && monthRelease > 0 && dayRelease > 0) {
                setReleaseField(`${yearRelease}-${(monthRelease >= 1 && monthRelease <= 9) ? `0${monthRelease}`
                    : monthRelease}-${(dayRelease >= 1 && dayRelease <= 9) ? `0${dayRelease}` : dayRelease}`)
        ...
        function updatecreateInActor() {
            ...
            if (idActorEditName.length >= 3) {
                if (idActor.length > 0) {
                    ...
                    setShowCast(false)
                    if (idActor === "0") {
                        actionInsertcreateInActor(idActorEditName, () => {
                            setIdActorEditName("")
                            setIdActorEdit("")
                            setShowCast(true)
                        ...
                        actionUpdatecreateInActor(idActor, idActorEditName, () => {
        ...
        async function deletecreateInActor() {
            const actorId = idActorDelete
            setIdActorDelete(-1)
            setShowCast(false)
            actionDeletecreateInActor(actorId, searchTextCast, () => {
        ...
        function updatecreateInCountry() {
            ...
            if (idCountryEditName.length >= 3) {
                if (idCountry.length > 0) {
                    ...
                    if (idCountry === "0") {
                        actionInsertcreateInCountry(idCountryEditName, () => {
                        ...
                        actionUpdatecreateInCountry(idCountry, idCountryEditName, () => {
        ...
        async function deletecreateInCountry() {
            const countryId = idCountryDelete
            setIdCountryDelete(-1)
            setShowCountry(false)
            actionDeletecreateInCountry(countryId, searchTextCountry, () => {
        ...
        function updatecreateInStream() {
            ...
            if (idStreamEditName.length >= 3) {
                if (idStream.length > 0) {
                    ...
                    setShowStream(false)
                    if (idStream === "0") {
                        actionInsertcreateInStream(idStreamEditName, () => {
                            setIdStreamEditName("")
                            setIdStreamEdit("")
                            setShowStream(true)
                        ...
                        actionUpdatecreateInStream(idStream, idStreamEditName, () => {
        ...
        async function deletecreateInStream() {
            const streamId = idStreamDelete
            setIdStreamDelete(-1)
            setShowStream(false)
            actionDeletecreateInStream(streamId, searchTextStream, () => {
                        ...
                        <Grid item xs={12}>
                            <Grid container spacing={2}>
                                <Grid item md={4} xs={12}>
                                    <FormControlFieldSelect labelValue="DIA" isDisabled={(monthRelease === 0 || yearRelease === 0)}
                                        valueDefault={dayRelease > 0 ? dayRelease : ""} selectList={days}
                                        onChangeForm={(e) => setDayRelease(e.target.value > 0 ? e.target.value : 0)} />
                                </Grid>
                                <Grid item md={4} xs={12}>
                                    <FormControlFieldSelect labelValue="MÊS" isDisabled={yearRelease === 0}
                                        valueDefault={monthRelease > 0 ? monthRelease : ""} selectList={months}
                                        onChangeForm={(e) => setMonthRelease(e.target.value.length > 0 ? parseInt(e.target.value, 10) : 0)} />
                                </Grid>
                                <Grid item md={4} xs={12}>
                                    <FormControlFieldSelect labelValue="ANO*" valueDefault={yearRelease > 0 ? yearRelease : ""}
                                        selectList={years} onChangeForm={(e) => setYearRelease(e.target.value.length > 0 ? parseInt(e.target.value, 10) : 0)} />
                                </Grid>
                            </Grid>
                        </Grid>
                    </Grid>
        ...
        function dialogEditDirector() {
            return (<Dialog fullWidth maxWidth="xs" open={idDirectorEdit.length > 0} onClose={() => setIdDirectorEdit(idDirectorEdit)}
                ...
                <DialogContent>
                    <Grid container spacing={2}>
                        <Grid item xs={12}>
                            <Grid container spacing={2}>
                                <Grid item xs={12}>
                                    <FormControlField labelValue="NOME*" valueDefault={idDirectorEditName} onChangeForm={(e) =>
                                        setIdDirectorEditName(e.target.value)} />
                                </Grid>
                            </Grid>
                        </Grid>
                    </Grid>
        ...
        function dialogDirectors() {
            return (<Dialog fullWidth maxWidth="sm" open={showDirector} onClose={() => setShowDirector(true)} aria-labelledby="form-dialog-title">
                <DialogActions style={{ marginLeft: 10, marginRight: 10 }}>
                    <FormControlField valueDefault={searchTextDirector} labelValue="DIGITE O NOME" onChangeForm={(e) =>
                        changeSearchDirector(e.target.value)} InputProps={{
                            endAdornment: <InputAdornment position="end">
                                {searchTextDirector.length > 0 && <IconButton edge="end" onClick={() => changeSearchDirector("")}>
                                                ...
                                                <TableCellStyle width={100} align="center">
                                                    <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                        <ButtonWarning sizeBtn='small' titleIcon={ICON_OBJECT_LIST.REMOVE_ICON}
                                                            actionClick={() => changeDirectors(false, row)} />
                                                    </ButtonGroup>
                                                </TableCellStyle>
                                            </TableRowStyle>
                                        ))}
                                        {(searchTextDirector.length >= 3 && directors) && directors.filter(di =>
                                            stateDirector.directors.filter(d => d._id === di._id).length === 0)
                                            .map((row, key) => (
                                                <TableRowStyle hover key={key}>
                                                    <TableCellStyle scope="row" align="left" style={{ fontWeight: 'bold' }}>
                                                        {row.name}
                                                    ...
                                                    <TableCellStyle width={100} align="center">
                                                        <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                            {row.enabledEdit && <ButtonSuccess sizeBtn='small' titleIcon={ICON_OBJECT_LIST.EDIT_ICON}
                                                                actionClick={() => setIdDirectorEdit(row._id)} />}
                                                            {row.enabledEdit && <ButtonPink sizeBtn='small' titleIcon={ICON_OBJECT_LIST.DELETE_ICON}
                                                                actionClick={() => setIdDirectorDelete(row._id)} />}
                                                            <ButtonSuccess sizeBtn='small' titleIcon={ICON_OBJECT_LIST.CHECK_ICON} actionClick={() =>
                                                                changeDirectors(true, row)} />
                                                        </ButtonGroup>
                                                    </TableCellStyle>
                                                </TableRowStyle>
                                            ))}
        ...
        function dialogEditActor() {
            return (<Dialog fullWidth maxWidth="xs" open={idActorEdit.length > 0} onClose={() => setIdActorEdit(idActorEdit)}
                aria-labelledby="form-dialog-title">
                <DialogContent>
                    <Grid container spacing={2}>
                        <Grid item xs={12}>
                            <Grid container spacing={2}>
                                <Grid item xs={12}>
                                    <FormControlField labelValue="NOME*" valueDefault={idActorEditName} onChangeForm={(e) =>
                                        setIdActorEditName(e.target.value)} />
                                </Grid>
                            </Grid>
                        </Grid>
                    </Grid>
                ...
                <DialogActions>
                    <ButtonSuccess title="SALVAR" actionClick={() => updatecreateInActor()} />
        ...
        function dialogCast() {
            return (<Dialog fullWidth maxWidth="sm" open={showCast} onClose={() => setShowCast(true)} aria-labelledby="form-dialog-title">
                <DialogActions style={{ marginLeft: 10, marginRight: 10 }}>
                    <FormControlField valueDefault={searchTextCast} labelValue="DIGITE O NOME" onChangeForm={(e) =>
                        changeSearchCast(e.target.value)} InputProps={{
                            endAdornment: <InputAdornment position="end">
                                {searchTextCast.length > 0 && <IconButton edge="end" onClick={() => changeSearchCast("")}>
                                    <IconList icon={ICON_OBJECT_LIST.CLEAR_ICON} /></IconButton>}
                                        ...
                                        {stateCast.casts.map((row, key) => (
                                            <TableRowStyle hover key={key}>
                                                ...
                                                <TableCellStyle width={100} align="center">
                                                    <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                        <ButtonWarning sizeBtn='small' titleIcon={ICON_OBJECT_LIST.REMOVE_ICON}
                                                            actionClick={() => changeCasts(false, row)} />
                                                    </ButtonGroup>
                                                </TableCellStyle>
                                            </TableRowStyle>
                                        ))}
                                        {(searchTextCast.length >= 3 && actors) && actors.filter(ac =>
                                            stateCast.casts.filter(c => c._id === ac._id).length === 0)
                                            .map((row, key) => (
                                                <TableRowStyle hover key={key}>
                                                    <TableCellStyle scope="row" align="left" style={{ fontWeight: 'bold' }}>
                                                        {row.name}
                                                    ...
                                                    <TableCellStyle width={100} align="center">
                                                        <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                            {row.enabledEdit && <ButtonSuccess sizeBtn='small'
                                                                titleIcon={ICON_OBJECT_LIST.EDIT_ICON} actionClick={() => setIdActorEdit(row._id)} />}
                                                            {row.enabledEdit && <ButtonPink sizeBtn='small'
                                                                titleIcon={ICON_OBJECT_LIST.DELETE_ICON}
                                                                actionClick={() => setIdActorDelete(row._id)} />}
                                                            <ButtonSuccess sizeBtn='small' titleIcon={ICON_OBJECT_LIST.CHECK_ICON}
                                                                actionClick={() => changeCasts(true, row)} />
                                                        </ButtonGroup>
                                                    </TableCellStyle>
                                                </TableRowStyle>
                                            ))}
        ...
        function dialogCategories() {
            return (<Dialog fullWidth maxWidth="sm" open={showCategory} onClose={() => setShowCategory(true)} aria-labelledby="form-dialog-title">
                <DialogActions style={{ marginLeft: 10, marginRight: 10 }}>
                    <FormControlField valueDefault={searchTextCategory} labelValue="DIGITE O NOME" onChangeForm={(e) =>
                        changeSearchCategory(e.target.value)} InputProps={{
                            endAdornment: <InputAdornment position="end">
                                {searchTextCategory.length > 0 && <IconButton edge="end" onClick={() => changeSearchCategory("")}>
                                    <IconList icon={ICON_OBJECT_LIST.CLEAR_ICON} /></IconButton>}
                                        ...
                                        {stateCategory.categories.map((row, key) => (
                                            <TableRowStyle hover key={key}>
                                                ...
                                                <TableCellStyle width={100} align="center">
                                                    <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                        <ButtonWarning sizeBtn='small' titleIcon={ICON_OBJECT_LIST.REMOVE_ICON}
                                                            actionClick={() => changeCategories(false, row)} />
                                                    </ButtonGroup>
                                                </TableCellStyle>
                                            </TableRowStyle>
                                        ))}
                                        {(searchTextCategory.length >= 3 && categories) && categories.filter(ca =>
                                            stateCategory.categories.filter(c => c._id === ca._id).length === 0)
                                            .map((row, key) => (
                                                <TableRowStyle hover key={key}>
                                                    <TableCellStyle scope="row" align="left" style={{ fontWeight: 'bold' }}>
                                                        {row.name}
                                                    ...
                                                    <TableCellStyle width={50} align="center">
                                                        <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                            <ButtonSuccess sizeBtn='small' titleIcon={ICON_OBJECT_LIST.CHECK_ICON}
                                                                actionClick={() => changeCategories(true, row)} />
                                                        </ButtonGroup>
                                                    </TableCellStyle>
                                                </TableRowStyle>
                                            ))}
        ...
        function dialogEditCountry() {
            return (<Dialog fullWidth maxWidth="xs" open={idCountryEdit.length > 0} onClose={() => setIdCountryEdit(idCountryEdit)}
                aria-labelledby="form-dialog-title">
                <DialogContent>
                    <Grid container spacing={2}>
                        <Grid item xs={12}>
                            <Grid container spacing={2}>
                                <Grid item xs={12}>
                                    <FormControlField labelValue="NOME*" valueDefault={idCountryEditName} onChangeForm={(e) =>
                                        setIdCountryEditName(e.target.value)} />
                                </Grid>
                            </Grid>
                        </Grid>
                    </Grid>
                ...
                <DialogActions>
                    <ButtonSuccess title="SALVAR" actionClick={() => updatecreateInCountry()} />
        ...
        function dialogCountry() {
            return (<Dialog fullWidth maxWidth="sm" open={showCountry} onClose={() => setShowCountry(true)} aria-labelledby="form-dialog-title">
                <DialogActions style={{ marginLeft: 10, marginRight: 10 }}>
                    <FormControlField valueDefault={searchTextCountry} labelValue="DIGITE O NOME"
                        onChangeForm={(e) => changeSearchCountry(e.target.value)} InputProps={{
                            endAdornment: <InputAdornment position="end">
                                {searchTextCountry.length > 0 && <IconButton edge="end" onClick={() => changeSearchCountry("")}>
                                    <IconList icon={ICON_OBJECT_LIST.CLEAR_ICON} /></IconButton>}
                                        ...
                                        {stateCountry.countries.map((row, key) => (
                                            <TableRowStyle hover key={key}>
                                                ...
                                                <TableCellStyle width={100} align="center">
                                                    <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                        <ButtonWarning sizeBtn='small' titleIcon={ICON_OBJECT_LIST.REMOVE_ICON}
                                                            actionClick={() => changeCountry(false, row)} />
                                                    </ButtonGroup>
                                                </TableCellStyle>
                                            </TableRowStyle>
                                        ))}
                                        {(countries && searchTextCountry.length >= 3) && countries.filter(co =>
                                            stateCountry.countries.filter(c => c._id === co._id).length === 0)
                                            .map((row, key) => (
                                                <TableRowStyle hover key={key}>
                                                    <TableCellStyle scope="row" align="left" style={{ fontWeight: 'bold' }}>
                                                        {row.initial}
                                                    ...
                                                    <TableCellStyle width={100} align="center">
                                                        <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                            {row.enabledEdit && <ButtonSuccess sizeBtn='small' titleIcon={ICON_OBJECT_LIST.EDIT_ICON}
                                                                actionClick={() => setIdCountryEdit(row._id)} />}
                                                            {row.enabledEdit && <ButtonPink sizeBtn='small' titleIcon={ICON_OBJECT_LIST.DELETE_ICON}
                                                                actionClick={() => setIdCountryDelete(row._id)} />}
                                                            <ButtonSuccess sizeBtn='small' titleIcon={ICON_OBJECT_LIST.CHECK_ICON}
                                                                actionClick={() => changeCountry(true, row)} />
                                                        </ButtonGroup>
                                                    </TableCellStyle>
                                                </TableRowStyle>
                                            ))}
        ...
        function dialogEditStream() {
            return (<Dialog fullWidth maxWidth="xs" open={idStreamEdit.length > 0} onClose={() => setIdCountryEdit(idStreamEdit)}
                aria-labelledby="form-dialog-title">
                <DialogContent>
                    <Grid container spacing={2}>
                        <Grid item xs={12}>
                            <Grid container spacing={2}>
                                <Grid item xs={12}>
                                    <FormControlField labelValue="NOME*" valueDefault={idStreamEditName} onChangeForm={(e) =>
                                        setIdStreamEditName(e.target.value)} />
                                </Grid>
                            </Grid>
                        </Grid>
                    </Grid>
                ...
                <DialogActions>
                    <ButtonSuccess title="SALVAR" actionClick={() => updatecreateInStream()} />
        ...
        function dialogStream() {
            return (<Dialog fullWidth maxWidth="sm" open={showStream} onClose={() => setShowStream(true)} aria-labelledby="form-dialog-title">
                <DialogActions style={{ marginLeft: 10, marginRight: 10 }}>
                    <FormControlField valueDefault={searchTextStream} labelValue="DIGITE O NOME" onChangeForm={(e) =>
                        changeSearchStream(e.target.value)} InputProps={{
                            endAdornment: <InputAdornment position="end">
                                {searchTextStream.length > 0 && <IconButton edge="end" onClick={() => changeSearchStream("")}>
                                    <IconList icon={ICON_OBJECT_LIST.CLEAR_ICON} /></IconButton>}
                                        ...
                                        {stateStream.streams.map((row, key) => (
                                            <TableRowStyle hover key={key}>
                                                ...
                                                <TableCellStyle width={100} align="center">
                                                    <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                        <ButtonWarning sizeBtn='small' titleIcon={ICON_OBJECT_LIST.REMOVE_ICON}
                                                            actionClick={() => changeStream(false, row)} />
                                                    </ButtonGroup>
                                                </TableCellStyle>
                                            </TableRowStyle>
                                        ))}
                                        {(searchTextStream.length >= 3 && streams) && streams.filter(st =>
                                            stateStream.streams.filter(s => s._id === st._id).length === 0)
                                            .map((row, key) => (
                                                <TableRowStyle hover key={key}>
                                                    <TableCellStyle scope="row" align="left" style={{ fontWeight: 'bold' }}>
                                                        {row.name}
                                                    ...
                                                    <TableCellStyle width={100} align="center">
                                                        <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                            {row.enabledEdit && <ButtonSuccess sizeBtn='small'
                                                                titleIcon={ICON_OBJECT_LIST.EDIT_ICON} actionClick={() => setIdStreamEdit(row._id)} />}
                                                            {row.enabledEdit && <ButtonPink sizeBtn='small' titleIcon={ICON_OBJECT_LIST.DELETE_ICON}
                                                                actionClick={() => setIdStreamDelete(row._id)} />}
                                                            <ButtonSuccess sizeBtn='small' titleIcon={ICON_OBJECT_LIST.CHECK_ICON}
                                                                actionClick={() => changeStream(true, row)} />
                                                        </ButtonGroup>
                                                    </TableCellStyle>
                                                </TableRowStyle>
                                            ))}
        ...
        return (
            <PageCard title="EDITAR FILME" sizeXs={12}>
                    ...
                    <Grid item xs={12}>
                        <Card>
                            <CardActions>
                                <Grid container spacing={2}>
                                    <Grid item md={4} xs={12}>
                                        <FormControlField isDisabled={(!getMovie || isLoading)}
                                            labelValue="TÍTULO*" valueDefault={titleField}
                                            onChangeForm={(e) => setTitleField(e.target.value)} />
                                    </Grid>
                                    <Grid item md={3} xs={12}>
                                        <FormControlField isDisabled={(!getMovie || isLoading)}
                                            labelValue="LANÇAMENTO*" valueDefault={releaseStrField} InputProps={{
                                    ...
                                    <Grid item xs={2}>
                                        <FormControlFieldMask isDisabled={(!getMovie || isLoading)} valueMask='99:99'
                                            labelValue="DURAÇÃO*" valueDefault={durationField} onChangeForm={(e) => setDurationField(e.target.value)} />
                                    </Grid>
                                    <Grid item xs={5}>
                                    ...
                                    <Grid item md={1} xs={12}>
                                        <FormControlFieldSelect isDisabled={(!getMovie || isLoading)} labelValue="CINEMA*"
                                            valueDefault={movieTheaterField} selectList={[
                                                { label: 'SIM', value: 'YES' }, { label: 'NÃO', value: 'NOT' }
                                            ]} onChangeForm={(e) => setMovieTheaterField(e.target.value)} />
                                    </Grid>
                                    <Grid item md={5} xs={12}>
                                        <FormControlField isDisabled={(!getMovie || isLoading)} labelValue="SINOPSE"
                                            valueDefault={resumeField} onChangeForm={(e) => setResumeField(e.target.value)} />
                                    </Grid>
                                </Grid>
                            </CardActions>
                            <CardActions className={classes.button_end}>
                                <ButtonSuccess title="SALVAR" isDisabled={(!getMovie || isLoading)} iconStart={ICON_OBJECT_LIST.CHECK_ICON}
                                    actionClick={() => update(titleField, releaseField, stateDirector.directors.map(d => d._id),
                                        stateCast.casts.map(c => c._id), durationField, stateCategory.categories.map(c => c._id),
                                        stateCountry.countries.map(c => c._id), stateStream.streams.map(c => c._id),
                                        movieTheaterField, resumeField)} />
                ...
                {dialogEditStream()}
                {dialogStream()}
                <DialogYesOrNot showDialog={idDirectorDelete !== -1} onCloseDialog={() => setIdDirectorDelete(-1)}
                    clickDialogNot={() => setIdDirectorDelete(-1)} titleDialog={MSG_DELETE_REGISTER_QUESTION}
                    clickDialogYes={() => deleteRegisterDirector()} />
                <DialogYesOrNot showDialog={idActorDelete !== -1} onCloseDialog={() => setIdActorDelete(-1)}
                    clickDialogNot={() => setIdActorDelete(-1)} titleDialog={MSG_DELETE_REGISTER_QUESTION}
                    clickDialogYes={() => deletecreateInActor()} />
                <DialogYesOrNot showDialog={idCountryDelete !== -1} onCloseDialog={() => setIdCountryDelete(-1)}
                    clickDialogNot={() => setIdCountryDelete(-1)} titleDialog={MSG_DELETE_REGISTER_QUESTION}
                    clickDialogYes={() => deletecreateInCountry()} />
                <DialogYesOrNot showDialog={idStreamDelete !== -1} onCloseDialog={() => setIdStreamDelete(-1)}
                    clickDialogNot={() => setIdStreamDelete(-1)} titleDialog={MSG_DELETE_REGISTER_QUESTION}
                    clickDialogYes={() => deletecreateInStream()} />
            </PageCard>
        )
    }

export default MovieEditPageView