...
import {
    ...
    Dialog, DialogContent, DialogActions, Checkbox, FormControl, RadioGroup, FormControlLabel, Radio
} from "@mui/material"
...
import {
    MONTHS, MSG_APPROVED_REVIEWED_REGISTER_QUESTION, MSG_DELETE_REGISTERS_QUESTION, MSG_DELETE_REGISTER_QUESTION,
    URL_MOVIES,
    URL_MOVIE_EDIT, URL_MOVIE_NEW
...
const MovieListView: React.FC<{
    orderDefault: string, movies: any, positionPage: number, moviesBatch: any, countMovie: any, changePagination: any,
    getCategories: any, getCountries: any, actionOrderList: any, actionChangeSearchText: any, actionClickFilter: any,
    actionEraseFilter: any, actionRefreshList: any, actionDeleteRegister: any, actionDeleteBatch: any, actionApprovedRegister: any,
    showLoading: boolean, getListScroll: (valueScroll: number) => any[]
}> =
    function ({
        orderDefault, movies, moviesBatch, countMovie, positionPage, changePagination, getCategories, getCountries, actionOrderList,
        actionChangeSearchText, actionClickFilter, actionEraseFilter, actionRefreshList, actionDeleteRegister, actionDeleteBatch,
        ...
        function searchFinish(search: string) {
            changePagination(1)
            actionChangeSearchText(orderField, search, categoryFilter, releaseFilter, durationMinFilter,
        ...
        async function refreshList() {
            changePagination(1)
            actionRefreshList(orderField, searchText, categoryFilter, releaseFilter, durationMinFilter,
                durationMaxFilter, countryFilter)
        }
        ...
        function confirmRelease() {
            ...
            if (yearReleaseFilter > 0 && monthReleaseFilter > 0 && dayReleaseFilter === 0) {
                setReleaseFilterTemp(`${yearReleaseFilter}-${(monthReleaseFilter >= 1 && monthReleaseFilter <= 9)
                    ? `0${monthReleaseFilter}` : monthReleaseFilter}`)
            ...
            if (yearReleaseFilter > 0 && monthReleaseFilter > 0 && dayReleaseFilter > 0) {
                setReleaseFilterTemp(`${yearReleaseFilter}-${(monthReleaseFilter >= 1 && monthReleaseFilter <= 9) ?
                    `0${monthReleaseFilter}` : monthReleaseFilter}-${(dayReleaseFilter >= 1 && dayReleaseFilter <= 9) ?
                        `0${dayReleaseFilter}` : dayReleaseFilter}`)
                setReleaseStrFilterTemp(`${(dayReleaseFilter >= 1 && dayReleaseFilter <= 9) ? `0${dayReleaseFilter}`
                    : dayReleaseFilter} de ${months.filter(m => parseInt(m.value, 10) === monthReleaseFilter)[0].label.substring(0, 3)}. de ${yearReleaseFilter}`)
            }
            setShowReleaseFilter(false)
        }

        function dialogRelease() {
            return (<Dialog fullWidth maxWidth="sm" open={showReleaseFilter} onClose={() => setShowReleaseFilter(true)}
                            ...
                            <Grid container spacing={2}>
                                <Grid item md={4} xs={12}>
                                    <FormControlFieldSelect labelValue="DIA"
                                        isDisabled={(monthReleaseFilter === 0 || yearReleaseFilter === 0)}
                                        valueDefault={dayReleaseFilter > 0 ? dayReleaseFilter : ""} selectList={days}
                                        onChangeForm={(e) => setDayReleaseFilter(e.target.value > 0 ? e.target.value : 0)} />
                                </Grid>
                                <Grid item md={4} xs={12}>
                                    <FormControlFieldSelect labelValue="MÊS" isDisabled={yearReleaseFilter === 0}
                                        valueDefault={monthReleaseFilter > 0 ? monthReleaseFilter : ""} selectList={months}
                                        onChangeForm={(e) => setMonthReleaseFilter(e.target.value.length > 0 ? parseInt(e.target.value, 10) : 0)} />
                                </Grid>
                                <Grid item md={4} xs={12}>
        ...
        async function deleteRegister() {
            ...
            setIdDelete(-1)
            actionDeleteRegister(movieId, orderField, searchText, categoryFilter, releaseFilter,
        ...
        async function deleteBatch() {
            ...
            actionDeleteBatch(stateArrayDeleteBatch.arrayDeleteBatch, orderField, searchText, categoryFilter,
                releaseFilter, durationMinFilter, durationMaxFilter, countryFilter, () => dispatchArrayDeleteBatch({ type: 'erase' }))
        }
        ...
        function validateDuration() {
            ...
            if (durationMaxFilterTemp.length > 0) {
                ...
                statusDurationMax = (parseInt(minuteMaxStr, 10) >= 0 && parseInt(minuteMaxStr, 10) <= 59)
                    && `${hourMaxStr}:${minuteMaxStr}`.length === 5
            }
            if (statusDurationMin && statusDurationMax) {
                return !((parseInt(hourMaxStr, 10) < parseInt(hourMinStr, 10))
                    || ((parseInt(hourMaxStr, 10) === parseInt(hourMinStr, 10) && parseInt(minuteMaxStr, 10) < parseInt(minuteMinStr, 10))))
            }
            return false
        }
        ...
        function clickFilter() {
            ...
            actionClickFilter(orderField, searchText, categoryFilterTemp, releaseFilterTemp,
                durationMinFilterTemp, durationMaxFilterTemp, countryFilterTemp)
        }
        ...
        async function approvedRegister() {
            ...
            setIdReviewed("")
            actionApprovedRegister(movieId, orderField, searchText, categoryFilter, releaseFilter,
                durationMinFilter, durationMaxFilter, countryFilter)
        }
                    ...
                    <Table aria-label="customized table">
                        <TableHeadStyle>
                            <TableRow>
                                ...
                                <TableCellStyle width={(movies && movies.filter(m => (m.enabledApproved || m.enabledEdit)).length > 0) ? 75 : 50} />
                            </TableRow>
                        </TableHeadStyle>
                                    ...
                                    <TableCellStyle align="center">
                                        <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                            ...
                                            {row.enabledEdit ? <ButtonIndigo sizeBtn='small' actionClick={() =>
                                                navigate(`${URL_MOVIE_EDIT}/${row._id}`)} titleIcon={ICON_OBJECT_LIST.EDIT_ICON} /> :
                                                <ButtonIndigo sizeBtn='small' actionClick={() =>
                                                    navigate(`${URL_MOVIES}/${row._id}`)} titleIcon={ICON_OBJECT_LIST.REMOVE_RED_EYE_ICON} />}
            ...
            return (<>
                <CardActions className={classes.flex_center}>
                    ...
                    <ButtonSuccess style={{ marginLeft: 10 }} titleIcon={ICON_OBJECT_LIST.ADD_ICON} iconTitleSize="medium"
                        actionClick={() => navigate(URL_MOVIE_NEW)} />
                    <ButtonDanger isDisabled={(!moviesBatch || moviesBatch.length === 0)} titleIcon={ICON_OBJECT_LIST.DELETE_ICON}
                            ...
                            <Grid item md={3} xs={12}>
                                <FormControlField labelValue="CATEGORIA" valueDefault={categoryNameFilter} InputProps={{
                                    endAdornment: categoryNameFilter.length > 0 ? <InputAdornment position="end">
                                        <IconButton edge="end" onClick={() => eraseFilter("", releaseFilter,
                                            durationMinFilter, durationMaxFilter, countryFilter)}>
                            ...
                            <Grid item md={3} xs={12}>
                                <FormControlField labelValue="LANÇAMENTO" valueDefault={releaseStrFilter} InputProps={{
                                    endAdornment: releaseFilter.length > 0 ? <InputAdornment position="end">
                                        <IconButton edge="end" onClick={() => eraseFilter(categoryFilter, "",
                                            durationMinFilter, durationMaxFilter, countryFilter)}>
                                            <IconList icon={ICON_OBJECT_LIST.CLEAR_ICON} />
                                        </IconButton>
                                    </InputAdornment> : null
                            ...
                            <Grid item md={2} xs={12}>
                                <FormControlField labelValue="DURAÇÃO MÁX." valueDefault={durationMaxFilter} InputProps={{
                                    endAdornment: durationMaxFilter.length > 0 ? <InputAdornment position="end">
                                        <IconButton edge="end" onClick={() => eraseFilter(categoryFilter, releaseFilter,
                                            durationMinFilter, "", countryFilter)}>
                                            <IconList icon={ICON_OBJECT_LIST.CLEAR_ICON} />
                                        </IconButton>
                                    </InputAdornment> : null
                                }} />
                            ...
                            <Grid item md={2} xs={12}>
                                <FormControlField labelValue="ORIGEM" valueDefault={countryNameFilter} InputProps={{
                                    endAdornment: countryFilter.length > 0 ? <InputAdornment position="end">
                                        <IconButton edge="end" onClick={() => eraseFilter(categoryFilter, releaseFilter,
                                            durationMinFilter, durationMaxFilter, "")}>
                                            <IconList icon={ICON_OBJECT_LIST.CLEAR_ICON} />
                                        </IconButton>
                                    </InputAdornment> : null
                                }} />
            ...
            return (<Dialog fullWidth maxWidth="sm" open={isFilter} onClose={() => setIsFilter(true)} aria-labelledby="form-dialog-title">
                <DialogContent>
                    <Grid container spacing={2}>
                        ...
                        <Grid item md={3} xs={12}>
                            <FormControlFieldMask valueMask='99:99' labelValue="DUR. MÍN."
                                valueDefault={durationMinFilterTemp}
                        ...
                        <Grid item xs={12}>
                            <FormControl>
                                <RadioGroup aria-labelledby="demo-radio-buttons-group-label" value={orderField} name="radio-buttons-group">
                                    <FormControlLabel value="title" control={<Radio />} label="TÍTULO" onChange={() =>
                                        orderList("title")} />
                                    <FormControlLabel value="releaseUp" control={<Radio />} label="LANÇAMENTO CRES." onChange={() =>
                                        orderList("releaseUp")} />
                                    <FormControlLabel value="releaseDown" control={<Radio />} label="LANÇAMENTO DEC." onChange={() =>
                                        orderList("releaseDown")} />
                                    <FormControlLabel value="durationUp" control={<Radio />} label="DURAÇÃO CRES." onChange={() =>
                                        orderList("durationUp")} />
                                    <FormControlLabel value="durationDown" control={<Radio />} label="DURAÇÃO DEC." onChange={() =>
                                        orderList("durationDown")} />
                                </RadioGroup>
                            </FormControl>
                        </Grid>
                    </Grid>
                        ...
                        <Grid item xs={12}>
                            <ScrollInfiniteList isShowScroll={(!moviesBatch || (positionScrollBatch * valueInitialScrollBatch) < moviesBatch.length)}
                                ...
                                <TableContainer component={Paper}>
                                    <Table aria-label="customized table">
                                        <TableBody>
                                            {moviesBatch && getListScroll((positionScrollBatch * valueInitialScrollBatch))
                ...
                <Grid container spacing={2}>
                    <Grid item xs={12}>
                        <Card>
                            {filterListMovies()}
                            {getListMovies()}
                            <PaginationList isLoading={showLoading} valuePage={positionPage} dataList={movies}
                                countList={countMovie} style={{ margin: 5 }} actionClick={(value: number) => changePagination(value)} />
                        </Card>
                    </Grid>
                </Grid>
