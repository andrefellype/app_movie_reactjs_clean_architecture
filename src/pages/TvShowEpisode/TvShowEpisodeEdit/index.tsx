...
function TvShowEpisodeEdit() {
    ...
    async function refreshOpenSeason() {
        if (typeof tvShowSeasonId !== "undefined" && tvShowSeasonId !== null) {
            ...
            await dispatch(openTvShowSeasonById(tvShowSeasonId, () => dispatch(showLoading(false)), (errorsMsg) => {
                dispatch(insertMsgs(errorsMsg, 'error'))
                dispatch(showLoading(false))
            }))
        }
    ...
    async function refreshOpenEpisode() {
        if (typeof tvShowEpisodeId !== "undefined" && tvShowEpisodeId !== null) {
            await dispatch(showLoading(true))
            await dispatch(openTvShowEpisodeById(tvShowEpisodeId, () => dispatch(showLoading(false)), (errorsMsg) => {
                dispatch(insertMsgs(errorsMsg, 'error'))
                dispatch(showLoading(false))
            }))
        }
