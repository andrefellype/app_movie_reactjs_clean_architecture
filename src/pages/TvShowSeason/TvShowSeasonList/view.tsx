...
import { MSG_APPROVED_REVIEWED_REGISTER_QUESTION, MSG_DELETE_REGISTERS_QUESTION, MSG_DELETE_REGISTER_QUESTION, URL_TV_SHOWS, URL_TV_SHOW_EPISODES, URL_TV_SHOW_SEASON_EDIT, URL_TV_SHOW_SEASON_NEW } from '../../../app/core/consts'
...
const TvShowSeasonListView: React.FC<{ getTvShow: any, seasons: any, actionRefreshList: any, actionChangeSearchText: any, actionDeleteRegister: any, actionDeleteBatch: any, actionApprovedRegister: any, showLoading: boolean, getListScroll: (valueScroll: number) => any[] }> =
    function ({ getTvShow, seasons, actionRefreshList, actionChangeSearchText, actionDeleteRegister, actionDeleteBatch, actionApprovedRegister, showLoading, getListScroll }) {
        ...
        function filterListSeasons() {
            return (<CardActions className={classes.flex_center}>
                ...
                <ButtonDanger isDisabled={(!seasons || seasons.length === 0)} titleIcon={ICON_OBJECT_LIST.DELETE_ICON} iconTitleSize="medium" actionClick={() => setIsDeleteBatch(true)} />
                        ...
                        <Grid item xs={12}>
                            <ScrollInfiniteList isShowScroll={(!seasons || (positionScrollBatch * valueInitialScrollBatch) < seasons.length)}
                                ...
                                <TableContainer component={Paper}>
                                    <Table aria-label="customized table">
                                        <TableBody>
                                            {seasons && getListScroll((positionScrollBatch * valueInitialScrollBatch)).filter(row => row.enabledEdit).map((row, key) => (
                                                <TableRowStyle hover key={key}
