...
import { MONTHS, MSG_APPROVED_REVIEWED_REGISTER_QUESTION, MSG_DELETE_REGISTERS_QUESTION, MSG_DELETE_REGISTER_QUESTION, URL_TV_SHOWS, URL_TV_SHOW_EDIT, URL_TV_SHOW_NEW, URL_TV_SHOW_SEASONS } from '../../../app/core/consts'
...
const TvShowListPageView: React.FC<{
    orderDefault: string, tvShows: any, tvShowsBatch: any, positionPage: number, countTvShow: any, changePagination: any,
    ...
    actionRefreshList: any, actionDeleteRegister: any, actionDeleteBatch: any, actionApprovedRegister: any, showLoading: boolean,
    getListScroll: (valueScroll: number) => any[]
}> =
    function ({ orderDefault, tvShows, tvShowsBatch, positionPage, countTvShow, changePagination, getCategories, getCountries,
        actionOrderList, actionChangeSearchText, actionClickFilter, actionEraseFilter, actionRefreshList, actionDeleteRegister, actionDeleteBatch,
        ...
        function confirmRelease() {
            ...
            if (yearReleaseFilter > 0 && monthReleaseFilter > 0 && dayReleaseFilter > 0) {
                setReleaseFilterTemp(`${yearReleaseFilter}-${(monthReleaseFilter >= 1 && monthReleaseFilter <= 9)
                    ? `0${monthReleaseFilter}` : monthReleaseFilter}-${(dayReleaseFilter >= 1
                        && dayReleaseFilter <= 9) ? `0${dayReleaseFilter}` : dayReleaseFilter}`)
                setReleaseStrFilterTemp(`${(dayReleaseFilter >= 1 && dayReleaseFilter <= 9)
                    ? `0${dayReleaseFilter}` : dayReleaseFilter} de ${months.filter(m => parseInt(m.value, 10) === monthReleaseFilter)[0].label.substring(0, 3)}. de ${yearReleaseFilter}`)
            }
            setShowReleaseFilter(false)
        }
        ...
        async function deleteBatch() {
            ...
            actionDeleteBatch(stateArrayDeleteBatch.arrayDeleteBatch, orderField, searchText, categoryFilter,
                releaseFilter, countryFilter, () => dispatchArrayDeleteBatch({ type: 'erase' }))
        }
                                ...
                                <TableRowStyle hover key={key}>
                                    ...
                                    <TableCellStyle align="center">
                                        <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                            <ButtonPink sizeBtn='small' actionClick={() => navigate(`${URL_TV_SHOW_SEASONS}/${row._id}`)}
                                                title="TEMPORADAS" />
                                            {row.enabledApproved && <ButtonSuccess sizeBtn='small' actionClick={() => setIdReviewed(row._id)}
                                                titleIcon={ICON_OBJECT_LIST.CHECK_ICON} />}
                                            {row.enabledEdit ? <ButtonIndigo sizeBtn='small' actionClick={() => navigate(`${URL_TV_SHOW_EDIT}/${row._id}`)}
                                                titleIcon={ICON_OBJECT_LIST.EDIT_ICON} /> : <ButtonIndigo sizeBtn='small' actionClick={() => navigate(`${URL_TV_SHOWS}/${row._id}`)}
                                                    titleIcon={ICON_OBJECT_LIST.REMOVE_RED_EYE_ICON} />}
            ...
            return (<>
                <CardActions className={classes.flex_center}>
                    ...
                    <ButtonSuccess style={{ marginLeft: 10 }} titleIcon={ICON_OBJECT_LIST.ADD_ICON} iconTitleSize="medium"
                        actionClick={() => navigate(URL_TV_SHOW_NEW)} />
                    <ButtonDanger isDisabled={(!tvShowsBatch || tvShowsBatch.length === 0)} titleIcon={ICON_OBJECT_LIST.DELETE_ICON}
                        ...
                        <Grid item xs={12}>
                            <ScrollInfiniteList isShowScroll={(!tvShowsBatch || (positionScrollBatch * valueInitialScrollBatch) < tvShowsBatch.length)}
                                ...
                                <TableContainer component={Paper}>
                                    <Table aria-label="customized table">
                                        <TableBody>
                                            {tvShowsBatch && getListScroll((positionScrollBatch * valueInitialScrollBatch))
...
export default TvShowListPageView