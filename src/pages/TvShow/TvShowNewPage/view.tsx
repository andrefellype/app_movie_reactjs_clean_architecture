...
const TvShowNewPageView: React.FC<{
    streams: any, stream: any, actionChangeSearchStream: any, actionRefreshListStream: any, actionInsertcreateInStream: any,
    actionUpdatecreateInStream: any, actionDeletecreateInStream: any, openStream: any, countries: any, country: any,
    actionChangeSearchCountry: any, actionRefreshListCountry: any, actionInsertcreateInCountry: any, actionUpdatecreateInCountry: any,
    actionDeletecreateInCountry: any, openCountry: any, categories: any, actionRefreshListCategory: any, actionChangeSearchCategory: any,
    saveRegister: any
}> = function ({
    ...
    categories, actionRefreshListCategory, actionChangeSearchCategory, saveRegister }) {
        ...
        function confirmRelease() {
            ...
            if (yearRelease > 0 && monthRelease > 0 && dayRelease > 0) {
                setReleaseField(`${yearRelease}-${(monthRelease >= 1 && monthRelease <= 9) ? `0${monthRelease}` : monthRelease}-${(dayRelease >= 1
                    && dayRelease <= 9) ? `0${dayRelease}` : dayRelease}`)
        ...
        function dialogCountry() {
            return (<Dialog fullWidth maxWidth="sm" open={showCountry} onClose={() => setShowCountry(true)} aria-labelledby="form-dialog-title">
                <DialogActions style={{ marginLeft: 10, marginRight: 10 }}>
                    <FormControlField valueDefault={searchTextCountry} labelValue="DIGITE O NOME" onChangeForm={(e) =>
                        changeSearchCountry(e.target.value)} InputProps={{
                            endAdornment: <InputAdornment position="end">
        ...
        return (
            <PageCard title="NOVA SÉRIE" sizeXs={12}>
                        ...
                        <Card>
                            ...
                            <CardActions className={classes.button_end}>
                                <ButtonSuccess title="SALVAR" iconStart={ICON_OBJECT_LIST.CHECK_ICON} actionClick={() =>
                                    saveRegister(titleField, releaseField, stateCategory.categories.map(c => c._id),
                                        stateCountry.countries.map(c => c._id), stateStream.streams.map(s => s._id), resumeField)} />
                ...
                <DialogYesOrNot showDialog={idCountryDelete !== -1} onCloseDialog={() => setIdCountryDelete(-1)}
                    clickDialogNot={() => setIdCountryDelete(-1)}
                ...
                <DialogYesOrNot showDialog={idStreamDelete !== -1} onCloseDialog={() => setIdStreamDelete(-1)}
                    clickDialogNot={() => setIdStreamDelete(-1)} titleDialog={MSG_DELETE_REGISTER_QUESTION} clickDialogYes={() => deletecreateInStream()} />
            </PageCard>
        )
    }

export default TvShowNewPageView