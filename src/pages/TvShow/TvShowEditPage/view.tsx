...
const TvShowEditPageView: React.FC<{
    streams: any, stream: any, actionChangeSearchStream: any, actionRefreshListStream: any,
    actionInsertcreateInStream: any, actionUpdatecreateInStream: any, actionDeletecreateInStream: any,
    openStream: any, countries: any, country: any, actionChangeSearchCountry: any, actionRefreshListCountry: any,
    actionInsertcreateInCountry: any, actionUpdatecreateInCountry: any, actionDeletecreateInCountry: any,
    openCountry: any, categories: any, actionRefreshListCategory: any, actionChangeSearchCategory: any,
    getTvShow: any, isLoading: boolean, update: any
}> = function ({
    streams, stream, actionChangeSearchStream, actionRefreshListStream, actionInsertcreateInStream,
    actionUpdatecreateInStream, actionDeletecreateInStream, openStream, countries, country, actionChangeSearchCountry,
    actionRefreshListCountry, actionInsertcreateInCountry, actionUpdatecreateInCountry, actionDeletecreateInCountry,
    openCountry, categories, actionRefreshListCategory, actionChangeSearchCategory, getTvShow, isLoading, update
}) {
        ...
        React.useEffect(() => {
            ...
            for (let y = 0; y < 100; y++) {
                year.push({
                    label: ((new Date()).getFullYear() - y).toString(),
                    value: ((new Date()).getFullYear() - y).toString()
                })
            }
            setYears(year)
        }, [])
        ...
        React.useEffect(() => {
            ...
            if (yearRelease > 0 && monthRelease > 0) {
                const dayMonth = ConvertDate(`${monthRelease >= 10 ? monthRelease
                    : `0${monthRelease}`}_${yearRelease}`, "totalDayInMonthAndYear")
        ...
        React.useEffect(() => {
            if (getTvShow) {
                if (!getTvShow.enabledEdit) {
                    navigate(URL_TV_SHOWS)
                }
                setTitleField(getTvShow.title)
                setRelease()
                setResumeField(getTvShow.resume)
                dispatchCategory({
                    type: "replace", newCategories: getTvShow.categories.map(categoryV => {
                ...
                dispatchStream({
                    type: "replace", newStreams: getTvShow.streams.map(streamV => {
        ...
        function confirmRelease() {
            ...
            if (yearRelease > 0 && monthRelease > 0 && dayRelease === 0) {
                setReleaseField(`${yearRelease}-${(monthRelease >= 1 && monthRelease <= 9)
                    ? `0${monthRelease}` : monthRelease}`)
            ...
            if (yearRelease > 0 && monthRelease > 0 && dayRelease > 0) {
                ...
                setReleaseStrField(`${(dayRelease >= 1 && dayRelease <= 9) ? `0${dayRelease}`
                    : dayRelease} de ${months.filter(m => parseInt(m.value, 10) === monthRelease)[0].label.substring(0, 3)}. de ${yearRelease}`)
            }
            setShowRelease(false)
        }
        ...
        function dialogRelease() {
            return (<Dialog fullWidth maxWidth="sm" open={showRelease} onClose={() => setShowRelease(true)}
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item md={4} xs={12}>
                                    <FormControlFieldSelect labelValue="MÊS" isDisabled={yearRelease === 0}
                                        ...
                                        onChangeForm={(e) => setMonthRelease(e.target.value.length > 0 ?
                                            parseInt(e.target.value, 10) : 0)} />
                                </Grid>
                                <Grid item md={4} xs={12}>
                                    <FormControlFieldSelect labelValue="ANO*" valueDefault={yearRelease > 0 ?
                                        yearRelease : ""} selectList={years}
                                        onChangeForm={(e) => setYearRelease(e.target.value.length > 0
                                            ? parseInt(e.target.value, 10) : 0)} />
                                </Grid>
                            </Grid>
                        </Grid>
                    ...
                    <FormControlField valueDefault={searchTextCategory} labelValue="DIGITE O NOME"
                        onChangeForm={(e) => changeSearchCategory(e.target.value)} InputProps={{
                            endAdornment: <InputAdornment position="end">
                                {searchTextCategory.length > 0 && <IconButton edge="end"
                                    onClick={() => changeSearchCategory("")}>
                                    <IconList icon={ICON_OBJECT_LIST.CLEAR_ICON} /></IconButton>}
                                {searchTextCategory.length >= 3 && <IconButton edge="end"
                                    onClick={() => refreshListCategory()}>
                                    <IconList icon={ICON_OBJECT_LIST.REFRESH_ICON} />
                                </IconButton>}
                            </InputAdornment>
                        }} />
                </DialogActions>
                {(stateCategory.categories.length > 0 || searchTextCategory.length >= 3)
                    && <DialogContent>
                        <Grid container spacing={2}>
                            <Grid item xs={12}>
                                                ...
                                                <TableRowStyle hover key={key}>
                                                    ...
                                                    <TableCellStyle width={100} align="center">
                                                        <ButtonGroup variant="contained"
                                                            ...
                                                            <ButtonWarning sizeBtn='small'
                                                                titleIcon={ICON_OBJECT_LIST.REMOVE_ICON}
                                                                actionClick={() => changeCategories(false, row)} />
                                                        </ButtonGroup>
                                                    </TableCellStyle>
                                                </TableRowStyle>
                                            ))}
                                            {(searchTextCategory.length >= 3 && categories)
                ...
                <DialogActions style={{ marginLeft: 10, marginRight: 10 }}>
                    <FormControlField valueDefault={searchTextCountry} labelValue="DIGITE O NOME"
                        onChangeForm={(e) => changeSearchCountry(e.target.value)} InputProps={{
                            endAdornment: <InputAdornment position="end">
                                {searchTextCountry.length > 0 && <IconButton edge="end"
                                    onClick={() => changeSearchCountry("")}>
                                    <IconList icon={ICON_OBJECT_LIST.CLEAR_ICON} /></IconButton>}
                                {searchTextCountry.length >= 3 && <IconButton edge="end"
                                    onClick={() => refreshListCountry()}>
                                    <IconList icon={ICON_OBJECT_LIST.REFRESH_ICON} />
                                </IconButton>}
                            </InputAdornment>
                        }} />
                            ...
                            <TableContainer component={Paper}>
                                <Table aria-label="customized table">
                                    <TableBody>
                                        ...
                                        {(countries && searchTextCountry.length >= 3)
                                            && countries.filter(co => stateCountry.countries.filter(c => c._id === co._id).length === 0)
                                                .map((row, key) => (
                                                    <TableRowStyle hover key={key}>
                                                        <TableCellStyle scope="row" align="left"
                                                            style={{ fontWeight: 'bold' }}>
                ...
                <DialogActions style={{ marginLeft: 10, marginRight: 10 }}>
                    <FormControlField valueDefault={searchTextStream} labelValue="DIGITE O NOME"
                        onChangeForm={(e) => changeSearchStream(e.target.value)} InputProps={{
                            endAdornment: <InputAdornment position="end">
                                {searchTextStream.length > 0 && <IconButton edge="end"
                                    onClick={() => changeSearchStream("")}>
                                    <IconList icon={ICON_OBJECT_LIST.CLEAR_ICON} /></IconButton>}
                                {searchTextStream.length >= 3 && <IconButton edge="end"
                                    onClick={() => refreshListStream()}>
                                    <IconList icon={ICON_OBJECT_LIST.REFRESH_ICON} />
                                </IconButton>}
                            </InputAdornment>
                        }} />
                            ...
                            <TableContainer component={Paper}>
                                <Table aria-label="customized table">
                                    <TableBody>
                                        ...
                                        {(searchTextStream.length >= 3 && streams) &&
                                            streams.filter(st => stateStream.streams.filter(s => s._id === st._id).length === 0)
                                                .map((row, key) => (
                                                    <TableRowStyle hover key={key}>
        ...
        return (
            <PageCard title="EDITAR SÉRIE" sizeXs={12}>
                <Grid container spacing={2}>
                    <Grid item xs={12}>
                        <Card>
                            <CardActions>
                                <Grid container spacing={2}>
                                    <Grid item md={4} xs={12}>
                                        <FormControlField labelValue="TÍTULO*"
                                            isDisabled={(!getTvShow || isLoading)} valueDefault={titleField}
                                            onChangeForm={(e) => setTitleField(e.target.value)} />
                                    </Grid>
                                    <Grid item md={3} xs={12}>
                                        <FormControlField labelValue="LANÇAMENTO*"
                                            isDisabled={(!getTvShow || isLoading)} valueDefault={releaseStrField}
                                            InputProps={{
                                    ...
                                    <Grid item md={6} xs={12}>
                                        <FormControlField labelValue="SINOPSE"
                                            isDisabled={(!getTvShow || isLoading)}
                                            valueDefault={resumeField}
                                            onChangeForm={(e) => setResumeField(e.target.value)} />
                                    </Grid>
                                </Grid>
                            </CardActions>
                            <CardActions className={classes.button_end}>
                                <ButtonSuccess title="SALVAR" isDisabled={(!getTvShow || isLoading)}
                                    iconStart={ICON_OBJECT_LIST.CHECK_ICON} actionClick={() =>
                                        update(titleField, releaseField,
                                            stateCategory.categories.map(c => c._id),
                                            stateCountry.countries.map(c => c._id),
                                            stateStream.streams.map(s => s._id), resumeField)} />
                ...
                {dialogEditStream()}
                {dialogStream()}
                <DialogYesOrNot showDialog={idCountryDelete !== -1} onCloseDialog={() =>
                    setIdCountryDelete(-1)} clickDialogNot={() => setIdCountryDelete(-1)}
                    titleDialog={MSG_DELETE_REGISTER_QUESTION} clickDialogYes={() => deletecreateInCountry()} />
                <DialogYesOrNot showDialog={idStreamDelete !== -1} onCloseDialog={() =>
                    setIdStreamDelete(-1)} clickDialogNot={() => setIdStreamDelete(-1)}
                    titleDialog={MSG_DELETE_REGISTER_QUESTION} clickDialogYes={() => deletecreateInStream()} />
            </PageCard>
        )
    }

export default TvShowEditPageView